    ...
    public class BackingStore
    {
        ...
        public bool SaveProfileNew(int device, string proName)
        {
            ...
            try
            {
                ...
                {
                    // Enviar directamente al FTP
                    var ftpManager = new FtpManager(FtpConfig.Server, FtpConfig.Username, FtpConfig.Password);
                    bool ftpUploaded = ftpManager.UploadProfileContent(testStr, $"Profiles/{proName}{Global.XML_EXTENSION}");
                    if (ftpUploaded)
                    {
                        //AppLogger.LogToGui($"Profile {proName} uploaded to FTP successfully", false);
                    }
                    else
                    {
                        //AppLogger.LogToGui($"Failed to upload profile {proName} to FTP", true);
                        saved = false;
                    }
                }
                else
                {
                    // Guardar localmente si FTP está deshabilitado
            ...
            string propath = "", bool xinputChange = true, bool postLoad = true)
        {
            ...
            if (FtpConfig.EnableFtpUpload)
            {
                try
                {
                    ...
                    profileXml = ftpManager.DownloadProfileContent($"Profiles/{fileName}");
                    profileExists = !string.IsNullOrEmpty(profileXml);
                    if (profileExists)
                    {
                        //AppLogger.LogToGui($"Profile {fileName} loaded from FTP successfully", false);
                    }
                }
                catch (Exception ex)
                {
                    //AppLogger.LogToGui($"FTP download error: {ex.Message}", true);
                }
            }
        ...
        public bool LoadActions()
        {
            ...
            string actionsContent = null;
            
            // Intentar leer desde FTP primero
            if (FtpConfig.EnableFtpUpload)
            {
                try
                {
                    var ftpManager = new FtpManager(FtpConfig.Server, FtpConfig.Username, FtpConfig.Password);
                    actionsContent = ftpManager.DownloadProfileContent("actions.xml");
                }
                catch
                {
                    // Si falla FTP, usar archivo local
                }
            }
            
            // Si no se pudo obtener desde FTP, usar archivo local
            if (string.IsNullOrEmpty(actionsContent))
            {
                actionsContent = File.ReadAllText(m_Actions);
            }

            XmlSerializer serializer = new XmlSerializer(typeof(ActionsDTO));
            try
            {
                using StringReader sr = new StringReader(actionsContent);
